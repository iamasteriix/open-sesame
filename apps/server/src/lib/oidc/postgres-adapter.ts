...
export class OidcPostgresAdapter implements Adapter {
  ...
  async upsert (
    ...
    await dbPool.query({
      ...
      text: `
        insert into oidc_models (id, kind, payload, granted_at, expires_at)
        values ($1, $2, $3, $4, $5)
   ...
   * out, it is recommended (weasel talk) that confidential clients authenticate with other methods that
   * aren't `'none'` for enhanced security; these other methods require that `client_secret` be populated,
   * otherwise the provider will throw an error.
   * 
  ...
  /**
   * Flags one-time-use models so the provider can detect subsequent attempts
   */
  ...
  /**
   * Cleans up all models for a provided grant given its id.
   * Used, for example, when a user signs out or revokes consent.
   * Doesn't require `this.name` because it deletes every single record associated with the grant id.
   */
